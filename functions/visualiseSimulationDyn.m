function fig = visualiseSimulationDyn(simdata,staticPlot,view,alim)
    ...
    if ~staticPlot
        ...
        ax3 = nexttile(10,[1 3]);
        plot(ax3, t(1:end),angCtrl,LineWidth=2, Color=[0.8 0.1 0.1] ,DisplayName="Angular Velocity");
        title(ax3,"Angular Velocity"); ylabel("w (rad/s)"); grid on
    
        ax4 = nexttile(16,[1 3]);
        yyaxis left;
        plot(ax4, t(1:end),dlv,LineWidth=2, Color=[0.1 0.1 0.9] ,DisplayName="\Delta u_v ");
        ylabel("v (m/s^2)");
        hold(ax4,"on");
        yyaxis right
        plot(ax4, t(1:end),dav,LineWidth=2, Color=[0.8 0.1 0.1] ,DisplayName="\Delta u_{\omega}");
