function fig = visualiseSimulation(simdata,staticPlot,view,alim)
    ...
    if ~staticPlot
        ...
        x_obs_fp = obstacle1(3)*cos(draw_ang);
        y_obs_fp = obstacle1(3)*sin(draw_ang);
        plot(ax1,obstacle1(1)+x_obs_fp, obstacle1(2)+y_obs_fp,'k', 'LineWidth', 0.5)  % circle around SO    
        hold(ax1,"on")
        % Plot Obstacle 2
        x_obs_fp2 = obstacle2(3)*cos(draw_ang);
        y_obs_fp2 = obstacle2(3)*sin(draw_ang);
        plot(ax1,obstacle2(1)+x_obs_fp2, obstacle2(2)+y_obs_fp2,'k', 'LineWidth', 0.5)  % circle around SO    
