function visualiseSimulation(x_ol,x_cl,o_cl,SO_init,x_ref,N,rob_diameter)
%% Figure setup
figure(100)
...
for k = 1:step_size:size(x_ol,2)-1 % go through the open loop
    ...
    for i = 1:size(SO_init,1)
        ...
        plot(SO_init(i,1)+x_obs_fp, SO_init(i,2)+y_obs_fp,'k', 'LineWidth', 0.5)  % circle around SO
        hold on
    end
