...
xx(:,1) = current_state; % xx contains the history of states
t(1) = current_time;
...
main_loop = tic;
while(norm((current_state(1:3)-target_state),2) > 0.1 && mpciter < sim_tim / DT)
    args.p   = [current_state;target_state];                                         % p : parameter vector 9x1 [initial state ; target state]
    args.x0  = [reshape(X0',6*(N+1),1);reshape(u0',3*N,1)];     % initial value of the optimization variables
    ...
    u_cl= [u_cl ; u(1,:)];
    t(mpciter+1) = current_time;
                            %       tstep, t_now, x0, u, f, obstacle, cbfParms, r_veh
    [current_time, current_state, u0, u_qp] = simulateTimeStep(DT,    current_time,    current_state, u, f, obstacle, cbfParms, veh_rad, M );               % Apply the control and simulate the timestep
    
    xx(:,mpciter+2) = current_state;
